...
// ─── DATA ──────────────────────────────────────────────────
const trucks = [
  { id: 'C51', name: 'Caterpillar 777F', status: 'idle',  load: 0,   fuel: 62, route: 'Ожидание у Карьера 2', driver: 'Иванов И.И.',  idle: '18 мин' },
  { id: 'C48', name: 'Caterpillar 777F', status: 'ok',    load: 100, fuel: 88, route: 'Карьер 1 → Склад',      driver: 'Петров А.В.',  speed: 32 },
  { id: 'C14', name: 'Caterpillar 777F', status: 'alert', load: 0,   fuel: 31, route: 'Остановка — топливо',   driver: 'Сидоров К.П.' },
  ...
  { id: 'C11', name: 'Caterpillar 777F', status: 'idle',  load: 0,   fuel: 55, route: 'Ожидание у Карьера 1', driver: 'Новиков В.Г.', idle: '7 мин' },
]

const recommendations = [
  {
    id: 'R-047', truck: 'C51', priority: 'high',
    from: 'Карьер 2', to: 'Liebherr ER9350 №1',
    desc: 'Самосвал C51 простаивает 18 мин. Рекомендуется перенаправить к экскаватору Liebherr №1.',
    eff: 87, time: '17:13',
  },
  {
    id: 'R-046', truck: 'C11', priority: 'normal',
    from: 'Карьер 1', to: 'Caterpillar 395 №3',
...
const incidents = [
  { icon: '⛽', type: 'red',    title: 'Низкий уровень топлива',  desc: 'C14 — 31%, требуется дозаправка',                  time: '17:10' },
  { icon: '⚠️', type: 'yellow', title: 'Медленное движение',       desc: 'C48 снизил скорость до 8 км/ч на участке Б-3',    time: '17:08' },
  { icon: '🔧', type: 'yellow', title: 'Плановое ТО',              desc: 'Liebherr №2 — техобслуживание через 4 ч',          time: '17:05' },
  { icon: '📋', type: 'muted',  title: 'Смена водителя',           desc: 'C25 — Козлов Р.М. принял смену',                  time: '17:00' },
  { icon: '✅', type: 'muted',  title: 'Рекомендация принята',     desc: 'C40 направлен к Caterpillar 395 №3',              time: '16:58' },
]
...
const kpis = [
  ...
  { label: 'В работе',         value: 4,    color: 'green',  sub: '↑ +1 за час',    trend: 'up' },
  ...
  { label: 'Принято за смену', value: '34', color: 'green',  sub: '↑ 91% rate',     trend: 'up' },
]
...
function MapRoads() {
  return (
    <svg className={d['map-svg']} viewBox="0 0 800 450" preserveAspectRatio="none">
      ...
      {/* Road center dash */}
      <g stroke="var(--map-road-center)" strokeWidth="2" fill="none" strokeLinecap="round" strokeDasharray="7,9">
        <path d="M150 200 Q250 180 350 220 Q450 260 560 250"/>
        <path d="M350 220 Q380 300 420 360"/>
        <path d="M150 200 Q180 280 220 330"/>
        <path d="M560 250 Q620 200 680 160"/>
      ...
      {/* Quarry zones */}
      <ellipse cx="230" cy="185" rx="65" ry="45"
        stroke="var(--map-zone-stroke)" strokeWidth="1.5"
        fill="var(--map-zone-fill)" strokeDasharray="4,5"/>
      <ellipse cx="500" cy="240" rx="55" ry="40"
        stroke="var(--map-zone-stroke)" strokeWidth="1.5"
        fill="var(--map-zone-fill)" strokeDasharray="4,5"/>
      <ellipse cx="380" cy="350" rx="50" ry="35"
        stroke="var(--map-zone-stroke)" strokeWidth="1" opacity="0.7"
        fill="var(--map-zone-fill)" strokeDasharray="4,5"/>
      {/* Active route */}
      <path d="M224 189 Q250 178 350 220 Q400 238 482 242"
        stroke="rgba(246,83,24,0.55)" strokeWidth="2.5"
        fill="none" strokeDasharray="7,6" strokeLinecap="round"/>
      {/* Zone labels */}
      <text x="185" y="172" fill="var(--map-label)" fontSize="9"
        fontWeight="600" fontFamily="sans-serif" letterSpacing="1" textAnchor="middle">КАРЬЕР 1</text>
      <text x="500" y="228" fill="var(--map-label)" fontSize="9"
        fontWeight="600" fontFamily="sans-serif" letterSpacing="1" textAnchor="middle">КАРЬЕР 2</text>
      <text x="380" y="340" fill="var(--map-label)" fontSize="9"
        fontWeight="600" fontFamily="sans-serif" letterSpacing="1" textAnchor="middle">КАРЬЕР 3</text>
      <text x="680" y="155" fill="var(--map-label)" fontSize="9" opacity="0.7"
        fontWeight="600" fontFamily="sans-serif" letterSpacing="1" textAnchor="middle">СКЛАД</text>
      <text x="200" y="345" fill="var(--map-label)" fontSize="9" opacity="0.7"
        fontWeight="600" fontFamily="sans-serif" letterSpacing="1" textAnchor="middle">ОТВАЛ</text>
    </svg>
  )
}
    ...
    <div className={styles.layout}>
      ...
      <aside className={styles.sidebar}>
        ...
        <nav className={styles['sidebar__nav']}>
          ...
          <div className={styles['sidebar__section-label']} style={{ marginTop: 6 }}>Управление</div>
          {[
            ...
            <div key={item.label} className={styles['sidebar__item']}>
              ...
              {item.label}
              {item.badge && (
                <span className={styles['nav-badge']} style={{ background: 'var(--bg-card)', color: 'var(--text-muted)' }}>
                  {item.badge}
                </span>
              )}
            </div>
        ...
        <header className={styles.header}>
          ...
          <div className={styles['header__right']}>
            ...
            <div className={styles['dispatcher']}>
              ...
              <div className={styles['disp-name']}>Тимошева А.</div>
            </div>
          </div>
        </header>
          ...
          <div className={d['kpi-strip']}>
            {kpis.map(kpi => (
              <div key={kpi.label} className={d.kpi}>
                ...
                <div className={`${d['kpi__value']} ${kpi.color ? d[`kpi__value--${kpi.color}`] : ''}`}>
                  {kpi.value}
                </div>
                <div className={d['kpi__sub']}>
                  {kpi.trend
                    ? <><span className={d[`trend--${kpi.trend}`]}>{kpi.sub.split(' ')[0]}</span>{' '}{kpi.sub.split(' ').slice(1).join(' ')}</>
                    : kpi.sub
                  }
                </div>
              </div>
                  ...
                  <tbody>
                    {trucks.map(t => (
                      <tr
                        key={t.id}
                        className={
                          t.status === 'idle'  ? d['row--idle']  :
                          t.status === 'alert' ? d['row--alert'] : ''
                        }
                      >
                        ...
                        <td>
                          ...
                          {t.status === 'idle'  && <span className="badge badge--yellow"><span className="dot dot--yellow" />Простой {(t as any).idle}</span>}
          ...
          <div className={d['panel-right']}>
            ...
            <div className={d.recommendations}>
              <div className={d['section-header']}>
                ...
                <span className={d['badge-count']} style={{ background: 'var(--orange)' }}>
                ...
                {recommendations.map(rec => (
                  ...
                  >
                    <div className={d['rec-card__header']}>
                      <div className={`${d['rec-priority']} ${d[`rec-priority--${rec.priority}`]}`} />
            ...
            <div className={d.incidents}>
              <div className={d['section-header']}>
                ...
                <span className={d['badge-count']} style={{ background: 'var(--red)' }}>
                  {alertCount}
                </span>
                <div className={d['ml-auto']}>
